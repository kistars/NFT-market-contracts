...
import {RedBlackTreeLibrary, Price} from "./libraries/LibRedBlackTree.sol";
...
contract OrderStorage is Initializable {
        ...
        returns (Price nextBestPrice)
    {
        if (RedBlackTreeLibrary.isEmpty(price)) {
            nextBestPrice =
                (side == LibOrder.Side.Bid) ? priceTrees[collection][side].last() : priceTrees[collection][side].first();
        } else {
    ...
    function _addOrder(LibOrder.Order memory order) internal returns (OrderKey orderKey) {
        ...
        if (LibOrder.isSentinel(orderQueue.head)) {
            // 队列是否初始化
            orderQueues[order.nft.collection][order.side][order // 创建新的队列
                .price] = LibOrder.OrderQueue(LibOrder.ORDERKEY_SENTINEL, LibOrder.ORDERKEY_SENTINEL);
        ...
        if (LibOrder.isSentinel(orderQueue.tail)) {
            ...
            order, LibOrder.ORDERKEY_SENTINEL);
        } else {
            // 队列不为空
    ...
    function _removeOrder(LibOrder.Order memory order) internal returns (OrderKey orderKey) {
        ...
        while (LibOrder.isNotSentinel(orderKey) && !found) {
            ...
            if (
                (dbOrder.order.maker == order.maker) && (dbOrder.order.saleKind == order.saleKind)
                    && (dbOrder.order.expiry == order.expiry) && (dbOrder.order.salt == order.salt)
                    && (dbOrder.order.nft.tokenId == order.nft.tokenId) && (dbOrder.order.nft.amount == order.nft.amount)
            ) {
    ...
    function getBestOrder(address collection, uint256 tokenId, LibOrder.Side side, LibOrder.SaleKind saleKind)
        external
        view
