...
let newProtocolShare, newESVault, EIP712Name, EIP712Version
...
describe("EasySwap Test", function () {
    ...
    describe("should initialize successfully", async () => {
        it("should initialize successfully", async () => {
            ...
            expect(info.name).to.eq(EIP712Name);
            expect(info.version).to.eq(EIP712Version);
        });
    });
    ...
    describe("should make order successfully", async () => {
        it("should make list/sell order successfully", async () => {
            ...
            expect(orderKeys[0]).to.not.eq(Byte32Zero);
            ...
            expect(await testERC721.ownerOf(0)).to.equal(esVault.address) // nft已经转入vault
        });
        ...
        it("should make two side order successfull", async () => {
            const now = parseInt(new Date() / 1000) + 100000
            const salt = 1;
            const nftAddress = testERC721.address;
            const tokenId = 0;
    ...
    describe("should cancel order successfully", async () => {
        it("should cancel list order successfully", async () => {
            ...
            // 从order map中获取
            dbOrder = await esDex.orders(orderHash);
            expect(dbOrder.order.maker).to.eq(owner.address);
            // 取消订单
            ...
            await expect(await esDex.cancelOrders([orderHash])).to.emit(esDex, "LogCancel")
